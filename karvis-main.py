...
def sendemail(to, content):
    ...
    server.ehlo()
    server.starttls()
    server.login('mail id (replace mail id  with ur id )', '(replace this with ur password )') # email id - use any email id whose security/privacy is off
    server.sendmail('(Replace with ur recivers id', to, content)
    server.close()
...
def wishme():
    ...
    speak("Myself KARVIS! How may I help you sir") #BotName - Give a name to your assistant
...
def play():
    ...
    while True:
        ...
        if 'exit' in query:
            ...
            music_dir = 'Music' # Enter the Path of Music Library
        ...
        elif 'who has created you' in query:
            var.set('My Creator is Dr.Manish Chandwani')
            window.update()
            speak('My Creator is Dr.Manish Chandwani')

        elif 'say hello' in query:
            var.set('Hello Everyone! My self Karvis')
            window.update()
            speak('Hello Everyone! My self Karvis')
     ...
     #   elif "open python" in query:
      #      var.set("Opening Python Ide")
       #     window.update()
         ...
         #   os.startfile["C:\Users\kartik's\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Python 3.7\\IDLE (Python 3.7 64-bit)"] #Enter the correct Path according to your system
...
window.title('KARVIS By ANAND SOFTWARES')

label = Label(window, width = 500, height = 500)
label.pack()
window.after(0, update, 0)

btn0 = Button(text = 'WISH ME',width = 20, command = wishme, bg = 'blue')
btn0.config(font=("Algerian", 12))
btn0.pack()
